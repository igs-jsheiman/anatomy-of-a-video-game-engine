...
namespace IGEngine
{
	...
	void RendererImpl::LoadShader()
	{
		...
		// Set up the vertex buffers here for now since this shader is very basic and not doing anything interesting
		{
			...
			CD3DX12_RESOURCE_DESC buf = CD3DX12_RESOURCE_DESC::Buffer(vertexBufferSize);
			ensure(SUCCEEDED(mDevice->CreateCommittedResource(&props,
